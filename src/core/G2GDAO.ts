...
const DEFAULT_CONFIG: Partial<G2GDAOConfig> = {
  theme: 'dark',
...
export class G2GDAO extends EventEmitter {
  ...
  constructor(config: G2GDAOConfig) {
    ...
    this.config = { ...DEFAULT_CONFIG, ...config } as G2GDAOConfig;
   ...
   * Open login modal
   */
  async login(options?: { wallet?: string; chain?: ChainType }): Promise<User> {
  ...
  private setupModalListeners(): void {
    // Listen to modal events and re-emit
  ...
  private getStorage(): Storage | null {
    ...
    switch (this.config.sessionStorage) {
      ...
      case 'cookie':
        // Cookie storage not implemented yet
        return window.localStorage;
      default:
        return window.localStorage;
    }
...
// Export as default for CDN usage
